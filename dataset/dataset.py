...
class PygNcDataset(InMemoryDataset):
    ...
    def process(self):
        name = self.name
        if name in ('Penn94', 'Amherst41', 'Cornell5', 'Johns Hopkins55', 'Reed98'):
            dataset = load_fb100_dataset(name)
        elif name == 'arxiv-year':
            dataset = load_arxiv_year_dataset()
        elif name == 'twitch-gamer':
            ...
            raise RuntimeError('No matching dataset')

        data = Data(dataset.graph['node_feat'],
                    dataset.graph['edge_index'],
                    dataset.graph['edge_feat'],
                    dataset.label
                    )

        num_nodes = data.y.shape[0]
        splits = dataset.get_idx_split()
        data.train_mask = torch.zeros(num_nodes).bool().index_put_((splits['train'],), torch.tensor(True))
        data.val_mask = torch.zeros(num_nodes).bool().index_put_((splits['valid'],), torch.tensor(True))
        data.test_mask = torch.zeros(num_nodes).bool().index_put_((splits['test'],), torch.tensor(True))
