...
class Net(torch.nn.Module):
    def __init__(self, dataset):
        ...
        self.filters = [create_filter(laplacian, b) for b in torch.arange(0, 2.1, 0.25)]
        self.D = torch.stack([f.mm(data.x) for f in self.filters], dim=2)
        self.x = self.D.matmul(C).squeeze()
        self.L = torch.stack(self.filters, dim=2).matmul(C).squeeze()
    ...
    def forward(self, data):
        L_hat = self.I - self.L
        x = F.relu(L_hat.mm(self.x).mm(self.W1))
